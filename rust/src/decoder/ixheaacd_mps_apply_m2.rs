extern "C" {
    ...
    fn ixheaacd_dec_interp_umx(
        m: *mut [WORD32; 28],
        ptr_r: *mut WORD32,
        ptr_m_prev: *mut WORD32,
        pstr_mps_state: *mut ia_heaac_mps_state_struct,
    ) -> VOID;
    fn ixheaacd_apply_abs_kernels(
        ptr_r_in: *mut WORD32,
        ptr_r_out: *mut WORD32,
        ptr_params: *mut size_t,
    ) -> VOID;
...
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_apply_m2(
    ...
    let mut p_array_struct: *mut ia_mps_dec_reuse_array_struct = (*pstr_mps_state)
        .array_struct;
    let mut p_m2_param: *mut ia_mps_dec_m2_param_struct = (*(*pstr_mps_state).aux_struct)
        .m2_param;
    let mut num_direct_signals: WORD32 = (*curr_state).num_direct_signals;
    let mut temp_1: WORD32 = 0;
    let mut loop_counter: WORD32 = 0;
    let mut col_counter: WORD32 = num_direct_signals + (*curr_state).num_decor_signals;
    let mut rout_ptr: *mut WORD32 = 0 as *mut WORD32;
    let mut rout_kernel_ptr: *mut WORD32 = 0 as *mut WORD32;
    ...
    let mut w_wet_real: *mut WORD32 = 0 as *mut WORD32;
    let mut w_wet_imag: *mut WORD32 = 0 as *mut WORD32;
    let mut w_dry_real: *mut WORD32 = 0 as *mut WORD32;
    let mut w_dry_imag: *mut WORD32 = 0 as *mut WORD32;
    ...
    let mut w_wet_offset: WORD32 = num_direct_signals * TSXHB;
    let mut num_output_channels: WORD32 = (*curr_state).num_output_channels;
    let mut time_slots: WORD32 = (*curr_state).time_slots;
    let mut hybrid_bands: WORD32 = (*curr_state).hybrid_bands;
    ...
    let mut residual_coding: WORD32 = (*curr_state).residual_coding;
    let mut index_ptr: *mut WORD32 = ((*curr_state).index).as_mut_ptr();
    let mut params: [size_t; 4] = [0; 4];
    params[0 as core::ffi::c_int as usize] = &mut *((*curr_state).kernels)
        .as_mut_ptr()
        .offset(0 as core::ffi::c_int as isize) as *mut size_t as size_t;
    params[1 as core::ffi::c_int as usize] = time_slots as size_t;
    params[2 as core::ffi::c_int as usize] = num_parameter_bands as size_t;
    params[3 as core::ffi::c_int as usize] = hybrid_bands as size_t;
    rout_ptr = (*pstr_mps_state).mps_scratch_mem_v as *mut WORD32;
    rout_kernel_ptr = rout_ptr
        .offset(
            ((5112 as usize)
    ...
    while row < num_output_channels {
        ...
        while ts < time_slots {
            memset(
                hyb_output_real_dry as *mut core::ffi::c_void,
                0 as core::ffi::c_int,
                (hybrid_bands as size_t)
                    .wrapping_mul(::core::mem::size_of::<WORD32>() as size_t),
            );
            memset(
                hyb_output_imag_dry as *mut core::ffi::c_void,
                0 as core::ffi::c_int,
                (hybrid_bands as size_t)
                    .wrapping_mul(::core::mem::size_of::<WORD32>() as size_t),
            );
            hyb_output_real_dry = hyb_output_real_dry.offset(MAX_HYBRID_BANDS as isize);
            hyb_output_imag_dry = hyb_output_imag_dry.offset(MAX_HYBRID_BANDS as isize);
            ts += 1;
        }
        ...
        while col < num_direct_signals {
                ...
                & 2 as core::ffi::c_int != 0
            {
                ...
                ixheaacd_dec_interp_umx(
                    ((*p_m2_param).m2_resid_real[fresh0 as usize]).as_mut_ptr(),
                    rout_ptr,
                    m2_resid_real_prev,
                    pstr_mps_state,
                );
                ...
                while ts < time_slots {
                    ...
                    while qs < hybrid_bands {
                        ...
                        w_dry_real = w_dry_real.offset(1);
                        *hyb_output_real_dry = ixheaac_add32_sat(
                            *hyb_output_real_dry,
                            temp_1,
                        );
                        ...
                        w_dry_imag = w_dry_imag.offset(1);
                        rout_kernel_ptr = rout_kernel_ptr.offset(1);
                        *hyb_output_imag_dry = ixheaac_add32_sat(
                            *hyb_output_imag_dry,
                            temp_1,
                        );
                    ...
                    p_hyb_out_dry_re = p_hyb_out_dry_re
                        .offset(MAX_HYBRID_BANDS as isize);
                    p_hyb_out_dry_im = p_hyb_out_dry_im
                        .offset(MAX_HYBRID_BANDS as isize);
                    ts += 1;
                }
                m2_resid_real_prev = m2_resid_real_prev
                    .offset(num_parameter_bands as isize);
            }
            p_buffer_real = p_buffer_real.offset(TSXHB as isize);
            p_buffer_imag = p_buffer_imag.offset(TSXHB as isize);
            col += 1;
        }
        while col < loop_counter {
            let mut index: WORD32 = 0;
            let mut res: WORD32 = ixheaacd_get_res_idx(pstr_mps_state, col);
            index = *index_ptr.offset(res as isize);
            if (*curr_state).m2_param_present[row as usize][col as usize]
                & 2 as core::ffi::c_int != 0
            {
                let mut p_dry_real: *mut WORD32 = ((*p_array_struct).w_dry_real)
                    .offset((res as core::ffi::c_int * TSXHB) as isize);
                let mut p_dry_imag: *mut WORD32 = ((*p_array_struct).w_dry_imag)
                    .offset((res as core::ffi::c_int * TSXHB) as isize);
                let fresh1 = idx;
                idx = idx + 1;
                ixheaacd_dec_interp_umx(
                    ((*p_m2_param).m2_resid_real[fresh1 as usize]).as_mut_ptr(),
                    rout_ptr,
                    m2_resid_real_prev,
                    pstr_mps_state,
                );
                ...
                while ts < time_slots {
                    ...
                    rout_kernel_ptr = rout_kernel_ptr
                        .offset((hybrid_bands - index) as isize);
                    p_hyb_out_dry_re = p_hyb_out_dry_re
                        .offset(MAX_HYBRID_BANDS as isize);
                    p_hyb_out_dry_im = p_hyb_out_dry_im
                        .offset(MAX_HYBRID_BANDS as isize);
                    p_dry_real = p_dry_real.offset(MAX_HYBRID_BANDS as isize);
                    p_dry_imag = p_dry_imag.offset(MAX_HYBRID_BANDS as isize);
                    ts += 1;
                }
                m2_resid_real_prev = m2_resid_real_prev
                    .offset(num_parameter_bands as isize);
            ...
            while row < num_output_channels {
                ...
                while col < num_direct_signals {
                        ...
                        & 2 as core::ffi::c_int != 0
                    {
                        ...
                        idx = idx + 1;
                        ixheaacd_dec_interp_umx(
                            ((*p_m2_param).m2_resid_imag[fresh2 as usize]).as_mut_ptr(),
                            rout_ptr,
                            m2_resid_imag_prev,
                            pstr_mps_state,
                        );
                        ...
                        while ts < time_slots {
                            ...
                            while qs < 2 as core::ffi::c_int {
                                ...
                                *hyb_output_real_dry = ixheaac_add32_sat(
                                ...
                                w_dry_real = w_dry_real.offset(1);
                                rout_kernel_ptr = rout_kernel_ptr.offset(1);
                                *hyb_output_imag_dry = ixheaac_sub32_sat(
                                    *hyb_output_imag_dry,
                                    temp_1,
                                );
                                hyb_output_imag_dry = hyb_output_imag_dry.offset(1);
                            ...
                            while qs < hybrid_bands {
                                ...
                                w_dry_imag = w_dry_imag.offset(1);
                                *hyb_output_real_dry = ixheaac_sub32_sat(
                                    *hyb_output_real_dry,
                                    temp_1,
                                );
                                hyb_output_real_dry = hyb_output_real_dry.offset(1);
                ...
                while col < loop_counter {
                    let mut index_0: WORD32 = 0;
                    let mut res_0: WORD32 = ixheaacd_get_res_idx(pstr_mps_state, col);
                    index_0 = *index_ptr.offset(res_0 as isize);
                    if (*curr_state).m2_param_present[row as usize][col as usize]
                        & 2 as core::ffi::c_int != 0
                    {
                        let mut p_dry_real_0: *mut WORD32 = ((*p_array_struct)
                            .w_dry_real)
                            .offset((res_0 as core::ffi::c_int * TSXHB) as isize);
                        let mut p_dry_imag_0: *mut WORD32 = ((*p_array_struct)
                            .w_dry_imag)
                            .offset((res_0 as core::ffi::c_int * TSXHB) as isize);
                        let fresh3 = idx;
                        idx = idx + 1;
                        ixheaacd_dec_interp_umx(
                            ((*p_m2_param).m2_resid_imag[fresh3 as usize]).as_mut_ptr(),
                            rout_ptr,
                            m2_resid_imag_prev,
                            pstr_mps_state,
                        );
                        ...
                        while ts < time_slots {
                            hyb_output_real_dry = p_hyb_out_dry_re;
                            hyb_output_imag_dry = p_hyb_out_dry_im;
                            w_dry_real = p_dry_real_0;
                            w_dry_imag = p_dry_imag_0;
                            qs = 0 as core::ffi::c_int as WORD32;
                            while qs < 2 as core::ffi::c_int {
                                temp_1 = ixheaacd_mps_mult32_shr_15(
                                    *w_dry_imag,
                            ...
                            rout_kernel_ptr = rout_kernel_ptr
                                .offset((hybrid_bands - index_0) as isize);
                            p_hyb_out_dry_re = p_hyb_out_dry_re
                                .offset(MAX_HYBRID_BANDS as isize);
                            p_hyb_out_dry_im = p_hyb_out_dry_im
                                .offset(MAX_HYBRID_BANDS as isize);
                            p_dry_real_0 = p_dry_real_0
                                .offset(MAX_HYBRID_BANDS as isize);
                            p_dry_imag_0 = p_dry_imag_0
                                .offset(MAX_HYBRID_BANDS as isize);
                            ts += 1;
                        }
                        m2_resid_imag_prev = m2_resid_imag_prev
    ...
    while row < num_output_channels {
        ...
        while ts < time_slots {
            memset(
                hyb_output_real_wet as *mut core::ffi::c_void,
                0 as core::ffi::c_int,
                (hybrid_bands as size_t)
                    .wrapping_mul(::core::mem::size_of::<WORD32>() as size_t),
            );
            memset(
                hyb_output_imag_wet as *mut core::ffi::c_void,
                0 as core::ffi::c_int,
                (hybrid_bands as size_t)
                    .wrapping_mul(::core::mem::size_of::<WORD32>() as size_t),
            );
            hyb_output_real_wet = hyb_output_real_wet.offset(MAX_HYBRID_BANDS as isize);
            hyb_output_imag_wet = hyb_output_imag_wet.offset(MAX_HYBRID_BANDS as isize);
            ts += 1;
        }
        p_buffer_real = p_buffer_real.offset(TSXHB as isize);
        p_buffer_imag = p_buffer_imag.offset(TSXHB as isize);
    ...
    row = 0 as core::ffi::c_int as WORD32;
    while row < num_output_channels {
        p_buf_real = ((*p_array_struct).buffer_real).offset(w_wet_offset as isize);
        p_buf_imag = ((*p_array_struct).buffer_imag).offset(w_wet_offset as isize);
        ...
        while col < col_counter {
                ...
                & 1 as core::ffi::c_int != 0
            {
                ...
                idx = idx + 1;
                ixheaacd_dec_interp_umx(
                    ((*p_m2_param).m2_decor_real[fresh4 as usize]).as_mut_ptr(),
                    rout_ptr,
                    m2_decor_real_prev,
                    pstr_mps_state,
                );
                ...
                while ts < time_slots {
                    ...
                    while qs < hybrid_bands {
                        ...
                        w_wet_real = w_wet_real.offset(1);
                        *hyb_output_real_wet = ixheaac_add32_sat(
                            *hyb_output_real_wet,
                            temp_1,
                        );
                        ...
                        w_wet_imag = w_wet_imag.offset(1);
                        rout_kernel_ptr = rout_kernel_ptr.offset(1);
                        *hyb_output_imag_wet = ixheaac_add32_sat(
                            *hyb_output_imag_wet,
                            temp_1,
                        );
        ...
        if m2_param_imag_present != 0 {
            ...
            while row < num_output_channels {
                m2_decor_imag_prev = m2_decor_imag_prev
                    .offset((num_parameter_bands * num_direct_signals) as isize);
                p_buf_real = ((*p_array_struct).buffer_real)
                    .offset(w_wet_offset as isize);
                ...
                while col < col_counter {
                        ...
                        & 1 as core::ffi::c_int != 0
                    {
                        ...
                        idx = idx + 1;
                        ixheaacd_dec_interp_umx(
                            ((*p_m2_param).m2_decor_imag[fresh5 as usize]).as_mut_ptr(),
                            rout_ptr,
                            m2_decor_imag_prev,
                            pstr_mps_state,
                        );
                        ...
                        while ts < time_slots {
                            ...
                            while qs < 2 as core::ffi::c_int {
                                ...
                                *hyb_output_real_wet = ixheaac_add32_sat(
                                ...
                                w_wet_real = w_wet_real.offset(1);
                                rout_kernel_ptr = rout_kernel_ptr.offset(1);
                                *hyb_output_imag_wet = ixheaac_sub32_sat(
                                    *hyb_output_imag_wet,
                                    temp_1,
                                );
                                hyb_output_imag_wet = hyb_output_imag_wet.offset(1);
                            ...
                            while qs < hybrid_bands {
                                ...
                                w_wet_imag = w_wet_imag.offset(1);
                                *hyb_output_real_wet = ixheaac_sub32_sat(
                                    *hyb_output_real_wet,
                                    temp_1,
                                );
                                hyb_output_real_wet = hyb_output_real_wet.offset(1);
