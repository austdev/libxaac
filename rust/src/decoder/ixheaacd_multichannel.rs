...
#[no_mangle]
pub unsafe extern "C" fn cblock_decode_huff_symbol(
    mut ptr_read_next: *mut UWORD8,
    mut bit_pos: WORD32,
    mut huff_ori: *const UWORD16,
    mut input: *mut WORD16,
    mut readword: *mut WORD32,
) -> WORD {
    let mut h: *const UWORD16 = 0 as *const UWORD16;
    let mut tot_bits: WORD = 0;
    let mut first_offset: UWORD16 = 0;
    let mut sign_ret_val: WORD16 = 0;
    let mut read_word1: UWORD32 = 0;
    read_word1 = (*readword << bit_pos) as UWORD32;
    h = huff_ori as *mut UWORD16;
    first_offset = 7 as UWORD16;
    h = h
        .offset(
            (read_word1 >> 32 as core::ffi::c_int - first_offset as core::ffi::c_int)
                as isize,
        );
    sign_ret_val = *h as WORD16;
    tot_bits = 0 as core::ffi::c_int as WORD;
    while sign_ret_val as core::ffi::c_int > 0 as core::ffi::c_int {
        tot_bits += first_offset as core::ffi::c_int;
        bit_pos += first_offset as core::ffi::c_int;
        bit_pos -= 8 as core::ffi::c_int;
        if bit_pos >= 0 as core::ffi::c_int {
            *readword = (*readword << 8 as core::ffi::c_int
        ...
        h = h
            .offset(
                (sign_ret_val as core::ffi::c_int & 0x7ff as core::ffi::c_int) as isize,
            );
        h = h
            .offset(
                (read_word1 >> 32 as core::ffi::c_int - first_offset as core::ffi::c_int)
    ...
    bit_pos
        += (sign_ret_val as core::ffi::c_int & 0x7fff as core::ffi::c_int)
            >> 11 as core::ffi::c_int;
    if bit_pos as core::ffi::c_int - 8 as core::ffi::c_int >= 0 as core::ffi::c_int {
        *readword = (*readword << 8 as core::ffi::c_int
            | *ptr_read_next as core::ffi::c_int) as WORD32;
    }
    *input = ((sign_ret_val as core::ffi::c_int & 0x7ff as core::ffi::c_int)
        - 60 as core::ffi::c_int) as WORD16;
    return tot_bits;
}
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_dec_coupling_channel_element(
    mut it_bit_buf: *mut ia_bit_buf_struct,
    mut aac_handle: *mut ia_aac_decoder_struct,
    mut samp_rate_idx: WORD32,
    mut ptr_aac_tables: *mut ia_aac_dec_tables_struct,
    mut common_tables_ptr: *mut ixheaacd_misc_tables,
    mut element_index_order: *mut WORD,
    mut ind_channel_info: *mut ia_enhaacplus_dec_ind_cc,
    ...
    let mut element_instance_tag: WORD32 = 0;
    let mut c: LOOPIDX = 0;
    let mut ind_sw_cce_flag: WORD = 0;
    let mut num_coupled_elements: WORD = 0;
    let mut num_gain_element_lists: WORD = 0 as WORD;
    let mut gain_element_scale: WORD = 0;
    ...
    let mut error_status: IA_ERRORCODE = IA_NO_ERROR;
    element_instance_tag = ixheaacd_read_bits_buf(it_bit_buf, 4 as WORD);
    *element_index_order.offset(0 as core::ffi::c_int as isize) = element_instance_tag
        as WORD;
    ind_sw_cce_flag = ixheaacd_read_bits_buf(it_bit_buf, 1 as WORD) as WORD;
    num_coupled_elements = ixheaacd_read_bits_buf(it_bit_buf, 3 as WORD) as WORD;
    c = 0 as core::ffi::c_int as LOOPIDX;
    while c < MAX_BS_ELEMENT {
        (*ind_channel_info).elements_coupled[c as usize] = -(1 as core::ffi::c_int)
            as WORD8;
        c += 1;
    }
    (*ind_channel_info).num_coupled_elements = num_coupled_elements;
    c = 0 as core::ffi::c_int as LOOPIDX;
    while c < num_coupled_elements as core::ffi::c_int + 1 as core::ffi::c_int {
        num_gain_element_lists += 1;
        (*ind_channel_info).cc_target_is_cpe[c as usize] = ixheaacd_read_bits_buf(
            it_bit_buf,
            1 as WORD,
        ) as WORD8;
        (*ind_channel_info).cc_target_tag_select[c as usize] = ixheaacd_read_bits_buf(
            it_bit_buf,
            4 as WORD,
        ) as WORD8;
        if (*ind_channel_info).cc_target_is_cpe[c as usize] != 0 {
            (*ind_channel_info).cc_l[c as usize] = ixheaacd_read_bits_buf(
                it_bit_buf,
                1 as WORD,
            ) as WORD8;
            (*ind_channel_info).cc_r[c as usize] = ixheaacd_read_bits_buf(
                it_bit_buf,
                1 as WORD,
            ) as WORD8;
            if (*ind_channel_info).cc_l[c as usize] as core::ffi::c_int != 0
                && (*ind_channel_info).cc_r[c as usize] as core::ffi::c_int != 0
            {
                num_gain_element_lists += 1;
            }
            (*ind_channel_info).elements_coupled[c as usize] = 1 as WORD8;
        } else {
            (*ind_channel_info).elements_coupled[c as usize] = 0 as WORD8;
        }
        c += 1;
    }
    if ind_sw_cce_flag == 0 as core::ffi::c_int
        && num_gain_element_lists > MAX_BS_ELEMENT
    {
        return IA_FATAL_ERROR as IA_ERRORCODE;
    }
    ixheaacd_read_bits_buf(it_bit_buf, 1 as WORD);
    ixheaacd_read_bits_buf(it_bit_buf, 1 as WORD);
    gain_element_scale = ixheaacd_read_bits_buf(it_bit_buf, 2 as WORD) as WORD;
    (*(*aac_handle).pstr_aac_dec_ch_info[0 as core::ffi::c_int as usize])
        .str_ics_info
        .num_swb_window = 0 as WORD16;
    (*(*aac_handle).pstr_aac_dec_ch_info[0 as core::ffi::c_int as usize])
        .str_ics_info
        .sampling_rate_index = samp_rate_idx as WORD16;
    (*(*aac_handle).pstr_aac_dec_ch_info[0 as core::ffi::c_int as usize])
        .common_window = 0 as WORD16;
    ...
    (*ind_channel_info).cc_gain[0 as core::ffi::c_int as usize] = ((1
        as core::ffi::c_int) << 29 as core::ffi::c_int) as WORD32;
    c = 1 as core::ffi::c_int as LOOPIDX;
    while c < num_gain_element_lists {
        let mut cge: WORD = 0;
        let mut common_gain_element_present: [WORD; 10] = [0; 10];
        let mut norm_value: WORD16 = 0;
        if ind_sw_cce_flag != 0 {
            cge = 1 as core::ffi::c_int as WORD;
        } else {
            common_gain_element_present[c as usize] = ixheaacd_read_bits_buf(
                it_bit_buf,
                1 as WORD,
            ) as WORD;
            cge = common_gain_element_present[c as usize];
            return IA_XHEAAC_DEC_EXE_FATAL_UNIMPLEMENTED_CCE as IA_ERRORCODE;
        }
        if cge != 0 {
            ...
            ixheaacd_aac_read_byte(&mut ptr_read_next, &mut bit_pos, &mut read_word);
            while bit_pos > 8 as core::ffi::c_int {
                ixheaacd_aac_read_byte(&mut ptr_read_next, &mut bit_pos, &mut read_word);
            }
            (*it_bit_buf).ptr_read_next = ptr_read_next;
            (*it_bit_buf).bit_pos = 7 as WORD32 - bit_pos;
            (*it_bit_buf).cnt_bits -= length as core::ffi::c_int;
            norm_value = (index as core::ffi::c_int - 60 as core::ffi::c_int) as WORD16;
            if norm_value as core::ffi::c_int == -(1 as core::ffi::c_int) {
                (*ind_channel_info).cc_gain[c as usize] = (*common_tables_ptr)
                    .cc_gain_scale[gain_element_scale as usize];
            } else {
                let mut i: core::ffi::c_int = 0;
                (*ind_channel_info).cc_gain[c as usize] = (*common_tables_ptr)
                    .cc_gain_scale[gain_element_scale as usize];
                i = 0 as core::ffi::c_int;
                while i < -(norm_value as core::ffi::c_int) - 1 as core::ffi::c_int {
                    (*ind_channel_info).cc_gain[c as usize] = ixheaac_mul32_sh(
                        (*ind_channel_info).cc_gain[c as usize],
                        (*common_tables_ptr).cc_gain_scale[gain_element_scale as usize],
                        29 as WORD8,
                    );
                    i += 1;
                }
...
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_dec_couple_channel(
    mut p_time_data: *mut WORD32,
    mut out_samp_cc: *mut WORD32,
    mut frame_size: WORD16,
    mut total_channels: WORD,
    mut gain_cc: WORD16,
) {
    let mut i: WORD = 0;
    let mut out_cc: WORD32 = 0;
    let mut ptr_out_samp: *mut WORD32 = &mut *out_samp_cc
        .offset(0 as core::ffi::c_int as isize) as *mut WORD32;
    i = (frame_size as core::ffi::c_int - 1 as core::ffi::c_int) as WORD;
    while i >= 0 as core::ffi::c_int {
        let fresh0 = ptr_out_samp;
        ptr_out_samp = ptr_out_samp.offset(1);
        out_cc = ixheaac_shl32_sat(ixheaac_mult32x16in32(*fresh0, gain_cc), 3 as WORD);
        *p_time_data = ixheaac_add32_sat(out_cc, *p_time_data);
        p_time_data = p_time_data.offset(total_channels as isize);
        i -= 1;
    }
}
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_dec_ind_coupling(
    mut p_obj_exhaacplus_dec: *mut ia_exhaacplus_dec_api_struct,
    mut coup_ch_output: *mut WORD32,
    mut frame_size: WORD16,
    mut total_channels: WORD,
    mut ptr_time_data_tmp: *mut core::ffi::c_void,
) -> IA_ERRORCODE {
    let mut c: WORD = 0;
    let mut j: WORD = 0;
    let mut k: WORD = 0;
    let mut l: WORD = 0;
    let mut out_samp_cc: *mut WORD32 = 0 as *mut WORD32;
    let mut err: WORD32 = 0 as WORD32;
    let mut ind_channel_info: *mut ia_enhaacplus_dec_ind_cc = 0
        as *mut ia_enhaacplus_dec_ind_cc;
    let mut ptr_time_data: *mut WORD32 = ptr_time_data_tmp as *mut WORD32;
    ind_channel_info = &mut (*(*p_obj_exhaacplus_dec).p_state_aac).ind_cc_info;
    out_samp_cc = coup_ch_output;
    if (*ind_channel_info).num_coupled_elements > 0 as core::ffi::c_int
        && coup_ch_output.is_null()
    {
        if (*(*p_obj_exhaacplus_dec).p_state_aac).ec_enable != 0 {
            (*ind_channel_info).num_coupled_elements = 0 as core::ffi::c_int as WORD;
    ...
    c = 0 as core::ffi::c_int as WORD;
    while c
        < (*ind_channel_info).num_coupled_elements as core::ffi::c_int
            + 1 as core::ffi::c_int
    {
        l = 0 as core::ffi::c_int as WORD;
        while l < MAX_BS_ELEMENT {
            if (*p_obj_exhaacplus_dec).aac_config.element_type[l as usize]
                == (*ind_channel_info).elements_coupled[c as usize] as core::ffi::c_int
                && (*p_obj_exhaacplus_dec).aac_config.element_instance_order[l as usize]
                    == (*ind_channel_info).cc_target_tag_select[c as usize]
                        as core::ffi::c_int
            {
                break;
            }
        ...
        if !(l == MAX_BS_ELEMENT) {
            k = (*p_obj_exhaacplus_dec).aac_config.slot_element[l as usize] as WORD;
            if (*ind_channel_info).cc_target_is_cpe[c as usize] as core::ffi::c_int
                == 0 as core::ffi::c_int
            {
                let mut p_time_data: *mut WORD32 = &mut *ptr_time_data.offset(k as isize)
                    as *mut WORD32;
                let mut gain_cc: WORD16 = ixheaac_round16(
                    (*ind_channel_info).cc_gain[j as usize],
                );
            ...
            if (*ind_channel_info).cc_target_is_cpe[c as usize] as core::ffi::c_int
                == 1 as core::ffi::c_int
            {
                if (*ind_channel_info).cc_l[c as usize] as core::ffi::c_int
                    == 1 as core::ffi::c_int
                {
                    let mut p_time_data_0: *mut WORD32 = &mut *ptr_time_data
                        .offset(k as isize) as *mut WORD32;
                    let mut gain_cc_0: WORD16 = ixheaac_round16(
                        (*ind_channel_info).cc_gain[j as usize],
                    );
                    ixheaacd_dec_couple_channel(
                        p_time_data_0,
                ...
                k = (*p_obj_exhaacplus_dec).aac_config.slot_element[l as usize] as WORD;
                if (*ind_channel_info).cc_r[c as usize] as core::ffi::c_int
                    == 1 as core::ffi::c_int
                {
                    let mut p_time_data_1: *mut WORD32 = &mut *ptr_time_data
                        .offset((k as core::ffi::c_int + 1 as core::ffi::c_int) as isize)
                        as *mut WORD32;
                    let mut gain_cc_1: WORD16 = ixheaac_round16(
                        (*ind_channel_info)
                            .cc_gain[(j as core::ffi::c_int + 1 as core::ffi::c_int)
                            as usize],
                    );
                    ixheaacd_dec_couple_channel(
                        p_time_data_1,
...
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_dec_downmix_to_stereo(
    mut p_obj_exhaacplus_dec: *mut ia_exhaacplus_dec_api_struct,
    mut frame_size: WORD16,
    mut total_elements: WORD,
    mut ptr_time_data: *mut WORD16,
    mut total_channels: WORD,
) {
    let mut i: LOOPIDX = 0;
    let mut j: LOOPIDX = 0;
    let mut k: WORD = 0 as WORD;
    if 5 as core::ffi::c_int == total_channels {
        k = 0 as core::ffi::c_int as WORD;
    }
    if 6 as core::ffi::c_int == total_channels {
        k = 1 as core::ffi::c_int as WORD;
    }
    if 7 as core::ffi::c_int == total_channels {
        k = 2 as core::ffi::c_int as WORD;
    }
    if 8 as core::ffi::c_int == total_channels {
        k = 3 as core::ffi::c_int as WORD;
    }
    j = 0 as core::ffi::c_int as LOOPIDX;
    while j < frame_size as core::ffi::c_int {
        let mut temp_l: WORD16 = 0 as WORD16;
        let mut temp_r: WORD16 = 0 as WORD16;
        ...
        while i < total_elements {
            if 0 as core::ffi::c_int
                ...
                || 3 as core::ffi::c_int
                    == (*p_obj_exhaacplus_dec).aac_config.element_type[i as usize]
            {
                temp_l = (temp_l as core::ffi::c_int
                    + (ixheaac_mult32x16in32(
                        (*(*p_obj_exhaacplus_dec).common_tables)
                            .down_mix_martix[k
                            as usize][0 as core::ffi::c_int
                            as usize][(*p_obj_exhaacplus_dec)
                            .aac_config
                            .slot_element[i as usize] as usize],
                        *ptr_time_data
                            .offset(
                                (j as WORD32 * total_channels as WORD32
                                    + (*p_obj_exhaacplus_dec)
                                        .aac_config
                                        .slot_element[i as usize]) as isize,
                            ),
                    ) >> 14 as core::ffi::c_int) as WORD16 as core::ffi::c_int)
                    as WORD16;
                temp_r = (temp_r as core::ffi::c_int
                    + (ixheaac_mult32x16in32(
                        (*(*p_obj_exhaacplus_dec).common_tables)
                            .down_mix_martix[k
                ...
                temp_r = (temp_r as core::ffi::c_int
                    + (ixheaac_mult32x16in32(
                        (*(*p_obj_exhaacplus_dec).common_tables)
                            ...
                            as usize][((*p_obj_exhaacplus_dec)
                            .aac_config
                            .slot_element[i as usize] + 1 as core::ffi::c_int) as usize],
                        *ptr_time_data
                            .offset(
                                (j as core::ffi::c_int * total_channels as core::ffi::c_int
                                    + (*p_obj_exhaacplus_dec)
                                        .aac_config
                                        .slot_element[i as usize] + 1 as core::ffi::c_int) as isize,
                            ),
                    ) >> 14 as core::ffi::c_int) as WORD16 as core::ffi::c_int)
                    as WORD16;
            }
        ...
        *ptr_time_data.offset((2 as LOOPIDX * j) as isize) = temp_l;
        *ptr_time_data
            .offset(
                (2 as core::ffi::c_int * j as core::ffi::c_int + 1 as core::ffi::c_int)
                    as isize,
            ) = temp_r;
        j += 1;
    }
