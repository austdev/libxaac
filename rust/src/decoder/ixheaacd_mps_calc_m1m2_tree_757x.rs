...
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_calc_m1m2_7571(
    ...
    while ps < num_parameter_sets {
        ixheaacd_param_2_umx_ps(
            ...
            0 as WORD32,
            ps,
            (*pstr_mps_state).res_bands[0 as core::ffi::c_int as usize],
        );
        ixheaacd_param_2_umx_ps(
            pstr_mps_state,
            h11_r,
        ...
        while pb < num_parameter_bands {
            ...
            (*m1_param)
                ...
                as usize][2 as core::ffi::c_int as usize][ps as usize][pb as usize] = ONE_IN_Q15
                as WORD32;
            (*m1_param)
            ...
            (*m1_param)
                ...
                as usize][5 as core::ffi::c_int as usize][ps as usize][pb as usize] = ONE_IN_Q15
                as WORD32;
            (*m1_param)
        ...
        memcpy(
            ((*m2_param).m2_decor_real[0 as core::ffi::c_int as usize][ps as usize])
                .as_mut_ptr() as *mut core::ffi::c_void,
            h12_l as *const core::ffi::c_void,
            (num_parameter_bands as size_t)
                .wrapping_mul(::core::mem::size_of::<WORD32>() as size_t),
        );
        memcpy(
            ((*m2_param).m2_decor_real[1 as core::ffi::c_int as usize][ps as usize])
                .as_mut_ptr() as *mut core::ffi::c_void,
            h22_l as *const core::ffi::c_void,
            (num_parameter_bands as size_t)
                .wrapping_mul(::core::mem::size_of::<WORD32>() as size_t),
        );
        memcpy(
            ((*m2_param).m2_decor_real[2 as core::ffi::c_int as usize][ps as usize])
                .as_mut_ptr() as *mut core::ffi::c_void,
            h12_r as *const core::ffi::c_void,
            (num_parameter_bands as size_t)
                .wrapping_mul(::core::mem::size_of::<WORD32>() as size_t),
        );
        memcpy(
            ((*m2_param).m2_decor_real[3 as core::ffi::c_int as usize][ps as usize])
                .as_mut_ptr() as *mut core::ffi::c_void,
            h22_r as *const core::ffi::c_void,
            (num_parameter_bands as size_t)
                .wrapping_mul(::core::mem::size_of::<WORD32>() as size_t),
        );
        pb = 0 as core::ffi::c_int as WORD32;
        while pb < num_parameter_bands {
            ...
            let fresh0 = idx;
            idx = idx + 1;
            (*m2_param).m2_resid_real[fresh0 as usize][ps as usize][pb as usize] = *h11_l
                .offset(pb as isize);
            if residual_coding != 0 {
                let fresh1 = idx;
                idx = idx + 1;
                (*m2_param).m2_resid_real[fresh1 as usize][ps as usize][pb as usize] = *h12_res_l
                    .offset(pb as isize);
            }
            let fresh2 = idx;
            idx = idx + 1;
            (*m2_param).m2_resid_real[fresh2 as usize][ps as usize][pb as usize] = *h21_l
                .offset(pb as isize);
            if residual_coding != 0 {
                let fresh3 = idx;
                idx = idx + 1;
                (*m2_param).m2_resid_real[fresh3 as usize][ps as usize][pb as usize] = *h22_res_l
                    .offset(pb as isize);
            }
            let fresh4 = idx;
            idx = idx + 1;
            (*m2_param).m2_resid_real[fresh4 as usize][ps as usize][pb as usize] = ONE_IN_Q15
                as WORD32;
            let fresh5 = idx;
            idx = idx + 1;
            (*m2_param).m2_resid_real[fresh5 as usize][ps as usize][pb as usize] = *h11_r
                .offset(pb as isize);
            if residual_coding != 0 {
                let fresh6 = idx;
                idx = idx + 1;
                (*m2_param).m2_resid_real[fresh6 as usize][ps as usize][pb as usize] = *h12_res_r
                    .offset(pb as isize);
            }
            let fresh7 = idx;
            idx = idx + 1;
            (*m2_param).m2_resid_real[fresh7 as usize][ps as usize][pb as usize] = *h21_r
                .offset(pb as isize);
            if residual_coding != 0 {
                let fresh8 = idx;
                idx = idx + 1;
                (*m2_param).m2_resid_real[fresh8 as usize][ps as usize][pb as usize] = *h22_res_r
                    .offset(pb as isize);
            }
            let fresh9 = idx;
            idx = idx + 1;
            (*m2_param).m2_resid_real[fresh9 as usize][ps as usize][pb as usize] = ONE_IN_Q15
                as WORD32;
            let fresh10 = idx;
            idx = idx + 1;
            (*m2_param).m2_resid_real[fresh10 as usize][ps as usize][pb as usize] = ONE_IN_Q15
                as WORD32;
            let fresh11 = idx;
            idx = idx + 1;
            (*m2_param).m2_resid_real[fresh11 as usize][ps as usize][pb as usize] = ONE_IN_Q15
                as WORD32;
            pb += 1;
        }
        ps += 1;
...
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_calc_m1m2_7572(
    mut pstr_mps_state: *mut ia_heaac_mps_state_struct,
) -> VOID {
    let mut p_aux_struct: *mut ia_mps_dec_auxilary_struct = (*pstr_mps_state).aux_struct;
    let mut m2_param: *mut ia_mps_dec_m2_param_struct = (*p_aux_struct).m2_param;
    ...
    while ps < num_parameter_sets {
        ...
        while pb < num_parameter_bands {
            ...
            let fresh12 = idx;
            idx = idx + 1;
            (*m2_param).m2_resid_real[fresh12 as usize][ps as usize][pb as usize] = ONE_IN_Q15
                as WORD32;
            let fresh13 = idx;
            idx = idx + 1;
            (*m2_param).m2_resid_real[fresh13 as usize][ps as usize][pb as usize] = *h21_l
                .offset(pb as isize);
            if residual_coding != 0 {
                let fresh14 = idx;
                idx = idx + 1;
                (*m2_param).m2_resid_real[fresh14 as usize][ps as usize][pb as usize] = *h22_res_l
                    .offset(pb as isize);
            }
            let fresh15 = idx;
            idx = idx + 1;
            (*m2_param).m2_resid_real[fresh15 as usize][ps as usize][pb as usize] = *h11_l
                .offset(pb as isize);
            if residual_coding != 0 {
                let fresh16 = idx;
                idx = idx + 1;
                (*m2_param).m2_resid_real[fresh16 as usize][ps as usize][pb as usize] = *h12_res_l
                    .offset(pb as isize);
            }
            let fresh17 = idx;
            idx = idx + 1;
            (*m2_param).m2_resid_real[fresh17 as usize][ps as usize][pb as usize] = ONE_IN_Q15
                as WORD32;
            let fresh18 = idx;
            idx = idx + 1;
            (*m2_param).m2_resid_real[fresh18 as usize][ps as usize][pb as usize] = *h21_r
                .offset(pb as isize);
            if residual_coding != 0 {
                let fresh19 = idx;
                idx = idx + 1;
                (*m2_param).m2_resid_real[fresh19 as usize][ps as usize][pb as usize] = *h22_res_r
                    .offset(pb as isize);
            }
            let fresh20 = idx;
            idx = idx + 1;
            (*m2_param).m2_resid_real[fresh20 as usize][ps as usize][pb as usize] = *h11_r
                .offset(pb as isize);
            if residual_coding != 0 {
                let fresh21 = idx;
                idx = idx + 1;
                (*m2_param).m2_resid_real[fresh21 as usize][ps as usize][pb as usize] = *h12_res_r
                    .offset(pb as isize);
            }
            let fresh22 = idx;
            idx = idx + 1;
            (*m2_param).m2_resid_real[fresh22 as usize][ps as usize][pb as usize] = ONE_IN_Q15
                as WORD32;
            let fresh23 = idx;
            idx = idx + 1;
            (*m2_param).m2_resid_real[fresh23 as usize][ps as usize][pb as usize] = ONE_IN_Q15
                as WORD32;
            pb += 1;
        }
        ps += 1;
