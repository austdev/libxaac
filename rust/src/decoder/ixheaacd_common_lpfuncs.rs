extern "C" {
    ...
    fn ixheaacd_fill_prog_config_slots(
...
#[inline]
unsafe extern "C" fn ixheaacd_aac_showbits_7(
    mut it_bit_buff: *mut ia_bit_buf_struct,
) -> UWORD32 {
    let mut v: *mut UWORD8 = (*it_bit_buff).ptr_read_next;
    let mut b: UWORD32 = 0 as UWORD32;
    let mut x: UWORD32 = 0;
    b = ((*v.offset(0 as core::ffi::c_int as isize) as WORD32) << 8 as core::ffi::c_int)
        as UWORD32;
    if (*it_bit_buff).bit_pos < 6 as core::ffi::c_int {
        b |= *v.offset(1 as core::ffi::c_int as isize) as WORD32 as UWORD32;
    }
    x = b << 15 as WORD32 + 8 as WORD32 - (*it_bit_buff).bit_pos;
    x = x >> 25 as core::ffi::c_int;
    return x;
}
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_get_channel_mask(
    mut p_obj_enhaacplus_dec: *mut ia_exhaacplus_dec_api_struct,
) -> WORD {
    let mut ixheaacd_drc_offset: WORD = 0 as WORD;
    let mut channel_mask: WORD = 0 as WORD;
    let mut flag1: WORD = 0 as WORD;
    let mut flag2: WORD = 0 as WORD;
    let mut ch_idx: WORD = 0;
    let mut ptr_slot_element: *mut WORD = ((*p_obj_enhaacplus_dec)
        .aac_config
        .slot_element)
        .as_mut_ptr() as *mut WORD;
    let mut ptr_element_type: *mut WORD = ((*p_obj_enhaacplus_dec)
        .aac_config
        .element_type)
        .as_mut_ptr() as *mut WORD;
    memset(
        ptr_slot_element as *mut core::ffi::c_void,
        0 as core::ffi::c_int,
        (::core::mem::size_of::<WORD>() as size_t).wrapping_mul(MAX_BS_ELEMENT as size_t),
    );
    ch_idx = 0 as core::ffi::c_int as WORD;
    while ch_idx < MAX_BS_ELEMENT {
        if *ptr_element_type.offset(ch_idx as isize) == 1 as core::ffi::c_int {
            channel_mask += 0x3 as core::ffi::c_int;
            *ptr_slot_element.offset(ch_idx as isize) = ixheaacd_drc_offset;
            ixheaacd_drc_offset += 2 as core::ffi::c_int;
            flag1 = (ch_idx as core::ffi::c_int + 1 as core::ffi::c_int) as WORD;
            break;
        } else {
            ch_idx += 1;
        }
    ...
    ch_idx = 0 as core::ffi::c_int as WORD;
    while ch_idx < MAX_BS_ELEMENT {
        if *ptr_element_type.offset(ch_idx as isize) == 0 as core::ffi::c_int {
            channel_mask += 0x4 as core::ffi::c_int;
            *ptr_slot_element.offset(ch_idx as isize) = ixheaacd_drc_offset;
            ixheaacd_drc_offset += 1 as core::ffi::c_int;
            flag2 = (ch_idx as core::ffi::c_int + 1 as core::ffi::c_int) as WORD;
            break;
        } else {
            ch_idx += 1;
        }
    ...
    ch_idx = 0 as core::ffi::c_int as WORD;
    while ch_idx < MAX_BS_ELEMENT {
        if *ptr_element_type.offset(ch_idx as isize) == 3 as core::ffi::c_int {
            channel_mask += 0x8 as core::ffi::c_int;
            *ptr_slot_element.offset(ch_idx as isize) = ixheaacd_drc_offset;
            ixheaacd_drc_offset += 1 as core::ffi::c_int;
            break;
        } else {
    ...
    while ch_idx < MAX_BS_ELEMENT {
        if *ptr_element_type.offset(ch_idx as isize) == 1 as core::ffi::c_int {
            channel_mask += 0x30 as core::ffi::c_int;
            *ptr_slot_element.offset(ch_idx as isize) = ixheaacd_drc_offset;
            ixheaacd_drc_offset += 2 as core::ffi::c_int;
            flag1 = (ch_idx as core::ffi::c_int + 1 as core::ffi::c_int) as WORD;
            break;
    ...
    while ch_idx < MAX_BS_ELEMENT {
        if *ptr_element_type.offset(ch_idx as isize) == 0 as core::ffi::c_int {
            channel_mask += 0x100 as core::ffi::c_int;
            *ptr_slot_element.offset(ch_idx as isize) = ixheaacd_drc_offset;
            ixheaacd_drc_offset += 1 as core::ffi::c_int;
            break;
        } else {
    ...
    while ch_idx < MAX_BS_ELEMENT {
        if *ptr_element_type.offset(ch_idx as isize) == 1 as core::ffi::c_int {
            channel_mask += 0x40 as core::ffi::c_int + 0x80 as core::ffi::c_int;
            *ptr_slot_element.offset(ch_idx as isize) = ixheaacd_drc_offset;
            ixheaacd_drc_offset += 2 as core::ffi::c_int;
            break;
        } else {
...
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_read_data_stream_element(
    mut it_bit_buff: *mut ia_bit_buf_struct,
    mut byte_align_bits: *mut WORD32,
    mut drc_handle: *mut ia_drc_dec_struct,
) -> VOID {
    let mut temp_bs: ia_bit_buf_struct = {
    ...
    let mut count: WORD32 = ixheaacd_read_bits_buf(it_bit_buff, 13 as WORD);
    let mut cnt: WORD32 = count & 0xff as WORD32;
    let mut start_pos: WORD32 = 0 as WORD32;
    if cnt == 255 as core::ffi::c_int {
        cnt += ixheaacd_read_bits_buf(it_bit_buff, 8 as WORD);
    }
    if (count as core::ffi::c_int & 0x100 as core::ffi::c_int) >> 8 as core::ffi::c_int
        != 0
    {
        ixheaacd_byte_align(it_bit_buff, byte_align_bits);
    }
    memcpy(
        &mut temp_bs as *mut ia_bit_buf_struct as *mut core::ffi::c_void,
        it_bit_buff as *const core::ffi::c_void,
        ::core::mem::size_of::<ia_bit_buf_struct>() as size_t,
    );
    start_pos = temp_bs.cnt_bits;
    if ixheaacd_read_bits_buf(&mut temp_bs, 8 as WORD) == DVB_ANC_DATA_SYNC_BYTE {
        let mut dmx_level_present: core::ffi::c_int = 0;
        let mut compression_present: core::ffi::c_int = 0;
        let mut coarse_gain_present: core::ffi::c_int = 0;
        let mut fine_grain_present: core::ffi::c_int = 0;
        ixheaacd_read_bits_buf(&mut temp_bs, 8 as WORD);
        ixheaacd_read_bits_buf(&mut temp_bs, 3 as WORD);
        dmx_level_present = ixheaacd_read_bits_buf(&mut temp_bs, 1 as WORD)
            as core::ffi::c_int;
        ixheaacd_read_bits_buf(&mut temp_bs, 1 as WORD);
        compression_present = ixheaacd_read_bits_buf(&mut temp_bs, 1 as WORD)
            as core::ffi::c_int;
        coarse_gain_present = ixheaacd_read_bits_buf(&mut temp_bs, 1 as WORD)
            as core::ffi::c_int;
        fine_grain_present = ixheaacd_read_bits_buf(&mut temp_bs, 1 as WORD)
            as core::ffi::c_int;
        if dmx_level_present != 0 {
            ixheaacd_read_bits_buf(&mut temp_bs, 8 as WORD);
        }
        if compression_present != 0 {
            ixheaacd_read_bits_buf(&mut temp_bs, 16 as WORD);
        }
        ...
        if (*drc_handle).dvb_anc_data_present == 0
            && temp_bs.cnt_bits >= 0 as core::ffi::c_int
        {
            (*drc_handle).dvb_anc_data_pos = start_pos;
            (*drc_handle).dvb_anc_data_present = 1 as core::ffi::c_int as FLAG;
        }
    }
    if (*it_bit_buff).cnt_bits < cnt << 3 as core::ffi::c_int {
        longjmp(
            (*(*it_bit_buff).xaac_jmp_buf).as_mut_ptr(),
            IA_XHEAAC_DEC_EXE_NONFATAL_INSUFFICIENT_INPUT_BYTES,
        );
    }
    (*it_bit_buff).ptr_read_next = ((*it_bit_buff).ptr_read_next).offset(cnt as isize);
    (*it_bit_buff).cnt_bits -= cnt << 3 as core::ffi::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_read_fill_element(
    mut it_bit_buff: *mut ia_bit_buf_struct,
    mut drc_dummy: *mut ia_drc_dec_struct,
    mut ptr_drc_dec: *mut ia_drc_dec_struct,
    ...
    if count as core::ffi::c_int - 15 as core::ffi::c_int == 0 as core::ffi::c_int {
        count = ixheaacd_read_bits_buf(it_bit_buff, 8 as WORD);
        count = (count as core::ffi::c_int + 14 as core::ffi::c_int) as WORD32;
    }
    if count > 0 as core::ffi::c_int {
        ...
        if extension_type == EXT_DYNAMIC_RANGE {
            (*ptr_drc_dec).drc_element_found = 1 as UWORD8;
            count -= ixheaacd_dec_drc_read_element(ptr_drc_dec, drc_dummy, it_bit_buff);
        }
        if EXT_SAC_DATA == extension_type {
            ...
            *mps_header = anc_type;
            ...
            while i < count as core::ffi::c_int - 1 as core::ffi::c_int {
                *mps_buffer.offset(i as isize) = ixheaacd_read_bits_buf(
                    it_bit_buff,
                    8 as WORD,
                ) as UWORD8;
                i += 1;
            }
            *mps_bytes = (count as core::ffi::c_int - 1 as core::ffi::c_int) as WORD32;
        } else {
...
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_get_element_index_tag(
    mut p_obj_enhaacplus_dec: *mut ia_exhaacplus_dec_api_struct,
    mut ch_idx1: WORD,
    mut ch_idx: *mut WORD,
    mut channel: *mut WORD,
    mut ele_idx_order: *mut WORD,
    mut total_elements: WORD,
    mut element_used: *mut WORD8,
    ...
    let mut element_tag: WORD = 0;
    let mut j: WORD = 0;
    let mut p_state_enhaacplus_dec: *mut ia_aac_dec_state_struct = (*p_obj_enhaacplus_dec)
        .p_state_aac;
    let mut it_bit_buff: *mut ia_bit_buf_struct = (*p_state_enhaacplus_dec)
        .ptr_bit_stream;
    let mut element_idx: WORD = 0;
    let mut element_type: WORD = 0;
    let mut aac_dec_handle: *mut ia_aac_decoder_struct = (*p_state_enhaacplus_dec)
        .pstr_aac_dec_info[ch_idx1 as usize];
    *ch_idx = ch_idx1;
    ...
    if (*it_bit_buff).cnt_bits < 3 as core::ffi::c_int {
        ...
        return IA_XHEAAC_DEC_EXE_NONFATAL_INSUFFICIENT_INPUT_BYTES as WORD16 as WORD32;
    }
    element_tag = ixheaacd_read_bits_buf(it_bit_buff, 7 as WORD);
    ixheaacd_read_bidirection(it_bit_buff, -(7 as WORD32));
    element_idx = (element_tag as core::ffi::c_int & 0xf as core::ffi::c_int) as WORD;
    element_type = (element_tag as core::ffi::c_int >> 4 as core::ffi::c_int
        & 0x7 as core::ffi::c_int) as WORD;
    (*p_obj_enhaacplus_dec).aac_config.str_prog_config.alignment_bits = (*it_bit_buff)
        .bit_pos;
    while element_type == 4 as core::ffi::c_int || element_type == 5 as core::ffi::c_int
        || element_type == 6 as core::ffi::c_int
    {
        let mut type_0: WORD = ixheaacd_read_bits_buf(it_bit_buff, 3 as WORD);
        if (*it_bit_buff).cnt_bits < 3 as core::ffi::c_int {
            (*it_bit_buff).cnt_bits = -(1 as core::ffi::c_int) as WORD32;
            return IA_XHEAAC_DEC_EXE_NONFATAL_INSUFFICIENT_INPUT_BYTES as WORD16
                as WORD32;
        }
        if type_0 == 4 as core::ffi::c_int {
        ...
        if type_0 == 5 as core::ffi::c_int {
            let mut error_code: WORD32 = 0 as WORD32;
            error_code = ixheaacd_decode_pce(
                it_bit_buff as *mut ia_bit_buf_struct,
                &mut (*p_obj_enhaacplus_dec).aac_config.ui_pce_found_in_hdr,
                &mut (*p_obj_enhaacplus_dec).aac_config.str_prog_config,
            ) as WORD32;
        ...
        if (*it_bit_buff).cnt_bits < 7 as core::ffi::c_int {
            (*it_bit_buff).cnt_bits = -(1 as core::ffi::c_int) as WORD32;
            return IA_XHEAAC_DEC_EXE_NONFATAL_INSUFFICIENT_INPUT_BYTES as WORD16
                as WORD32;
        }
        element_tag = ixheaacd_aac_showbits_7(it_bit_buff) as WORD;
        element_idx = (element_tag as core::ffi::c_int & 0xf as core::ffi::c_int)
            as WORD;
        element_type = (element_tag as core::ffi::c_int >> 4 as core::ffi::c_int
            & 0x7 as core::ffi::c_int) as WORD;
    }
    if total_elements == 2 as core::ffi::c_int && total_channels == 2 as core::ffi::c_int
        && (*(*p_state_enhaacplus_dec).p_config).ui_pce_found_in_hdr == 2 as UWORD32
        && ch_idx1 == 0 as core::ffi::c_int
    {
        ixheaacd_fill_prog_config_slots(p_state_enhaacplus_dec);
    }
    *channel = 1 as core::ffi::c_int as WORD;
    if element_type == 1 as core::ffi::c_int {
        *channel = 2 as core::ffi::c_int as WORD;
    }
    j = 0 as core::ffi::c_int as WORD;
    while j < total_elements {
        if (*p_obj_enhaacplus_dec).aac_config.element_type[j as usize] == element_type
            && element_idx == *ele_idx_order.offset(j as isize)
            && *element_used.offset(j as isize) as core::ffi::c_int
                == 0 as core::ffi::c_int
        {
            *ch_idx = j;
            *element_used.offset(j as isize) = 1 as WORD8;
            break;
        } else {
            j += 1;
        }
    ...
    if j == total_elements {
        ...
        ixheaacd_read_bidirection(
            it_bit_buff,
            ((*it_bit_buff).cnt_bits - (*it_bit_buff).size) as WORD16 as WORD32,
        );
