...
pub const SHAPE_STP: core::ffi::c_int = 1;
pub const SHAPE_GES: core::ffi::c_int = 2;
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_get_ch_idx(
    mut pstr_mps_state: *mut ia_heaac_mps_state_struct,
    mut row: WORD32,
    mut index: *mut WORD32,
) -> VOID {
    match (*pstr_mps_state).temp_shape_config {
        SHAPE_STP => {
            *index = (*(*pstr_mps_state).ia_mps_dec_mps_table.m1_m2_table_ptr)
                .idx_table
                .row_2_channel_stp[(*pstr_mps_state).tree_config as usize][row as usize];
        }
        SHAPE_GES => {
            *index = (*(*pstr_mps_state).ia_mps_dec_mps_table.m1_m2_table_ptr)
                .idx_table
                .row_2_channel_ges[(*pstr_mps_state).tree_config as usize][row as usize];
        }
        _ => {}
    };
}
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_get_res_idx(
    mut pstr_mps_state: *mut ia_heaac_mps_state_struct,
    mut row: WORD32,
) -> WORD32 {
    return (*(*pstr_mps_state).ia_mps_dec_mps_table.m1_m2_table_ptr)
        .idx_table
        .row_2_residual[(*pstr_mps_state).tree_config as usize][row as usize];
}
