...
pub const IA_XHEAAC_DEC_EXE_NONFATAL_INSUFFICIENT_INPUT_BYTES: core::ffi::c_int = 0x1804
    as core::ffi::c_int;
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_allocate_mem_persistent(
    mut p_obj_enhaacplus_dec: *mut ia_exhaacplus_dec_api_struct,
    mut p_state_enhaacplus_dec: *mut ia_aac_dec_state_struct,
    mut channels: WORD,
    mut persistent_used_total: *mut WORD,
    mut sbr_persistent_start: *mut WORD,
    mut ps_enable: WORD,
) -> VOID {
    let mut persistent_used: WORD = 0;
    let mut temp_persistent: *mut *mut WORD8 = &mut (*p_state_enhaacplus_dec)
        .aac_persistent_mem_v as *mut *mut core::ffi::c_void as *mut *mut WORD8;
    *temp_persistent = (*temp_persistent).offset(*persistent_used_total as isize);
    ...
    *sbr_persistent_start = *persistent_used_total;
    ...
    persistent_used = ixheaacd_getsize_sbr_persistent() as WORD;
    ...
    *persistent_used_total += persistent_used;
    let mut aac_persistent_mem: *mut ia_aac_persistent_struct = (*(*p_obj_enhaacplus_dec)
        .p_state_aac)
        .aac_persistent_mem_v as *mut ia_aac_persistent_struct;
    (*aac_persistent_mem).str_aac_decoder.pstr_aac_tables = &mut (*p_obj_enhaacplus_dec)
        .aac_tables;
    (*aac_persistent_mem).str_aac_decoder.pstr_common_tables = (*p_obj_enhaacplus_dec)
        .common_tables;
    ixheaacd_set_sbr_persistent_table_pointer(
        (*(*p_obj_enhaacplus_dec).p_state_aac).sbr_persistent_mem_v,
        &mut (*p_obj_enhaacplus_dec).str_sbr_tables,
        (*p_obj_enhaacplus_dec).common_tables,
    );
}
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_create_bit_buf(
    ...
    (*it_bit_buff).adts_header_present = 0 as core::ffi::c_int as WORD32;
    (*it_bit_buff).protection_absent = 0 as WORD8;
    (*it_bit_buff).pstr_adts_crc_info = &mut (*it_bit_buff).str_adts_crc_info;
    (*it_bit_buff).max_size = (*it_bit_buff).size;
    ixheaacd_adts_crc_open((*it_bit_buff).pstr_adts_crc_info);
    return it_bit_buff;
}
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_create_init_bit_buf(
    mut it_bit_buff: *mut ia_bit_buf_struct,
    mut ptr_bit_buf_base: *mut UWORD8,
    mut bit_buf_size: WORD32,
) -> VOID {
...
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_read_bidirection(
    mut it_bit_buff: *mut ia_bit_buf_struct,
    mut ixheaacd_drc_offset: WORD32,
) -> VOID {
    if ixheaacd_drc_offset != 0 as core::ffi::c_int {
        let mut byte_offset: WORD32 = 0;
        if (*it_bit_buff).cnt_bits < 0 as core::ffi::c_int
            || (*it_bit_buff).cnt_bits - ixheaacd_drc_offset < 0 as core::ffi::c_int
        ...
        byte_offset = (*it_bit_buff).bit_pos >> 3 as core::ffi::c_int;
        (*it_bit_buff).bit_pos = (*it_bit_buff).bit_pos
            - (byte_offset << 3 as core::ffi::c_int);
        if byte_offset != 0 {
            let mut ptr_read_next: *mut UWORD8 = 0 as *mut UWORD8;
            ptr_read_next = (*it_bit_buff).ptr_read_next;
            ptr_read_next = ptr_read_next.offset(-(byte_offset as isize));
