...
pub unsafe extern "C" fn ixheaacd_mps_apply_m1(
    ...
    let mut rout_real_ptr: *mut WORD32 = 0 as *mut WORD32;
    let mut rout_imag_ptr: *mut WORD32 = 0 as *mut WORD32;
    let mut rout_kernel_real_ptr: *mut WORD32 = 0 as *mut WORD32;
    let mut rout_kernel_imag_ptr: *mut WORD32 = 0 as *mut WORD32;
    let mut v_real: *mut WORD32 = 0 as *mut WORD32;
    let mut v_imag: *mut WORD32 = 0 as *mut WORD32;
    let mut x_real: *mut WORD32 = 0 as *mut WORD32;
    let mut x_imag: *mut WORD32 = 0 as *mut WORD32;
    let mut p_buffer_real: *mut WORD32 = 0 as *mut WORD32;
    let mut p_buffer_imag: *mut WORD32 = 0 as *mut WORD32;
    let mut p_v_real: *mut WORD32 = 0 as *mut WORD32;
    let mut p_v_imag: *mut WORD32 = 0 as *mut WORD32;
    let mut curr_state: *mut ia_heaac_mps_state_struct = pstr_mps_state;
    let mut m1_param_real_prev: *mut WORD32 = (*curr_state)
        .mps_persistent_mem
        .m1_param_real_prev;
    let mut m1_param_imag_prev: *mut WORD32 = (*curr_state)
        .mps_persistent_mem
        .m1_param_imag_prev;
    let mut imag_present: WORD32 = (*curr_state).m1_param_imag_present;
    let mut v_channels: WORD32 = (*curr_state).num_v_channels;
    let mut x_channels: WORD32 = (*curr_state).num_x_channels;
    let mut hybrid_bands: WORD32 = (*curr_state).hybrid_bands;
    let mut num_parameter_bands: WORD32 = (*curr_state).num_parameter_bands;
    let mut index: *mut WORD32 = ((*curr_state).index).as_mut_ptr();
    let mut p_x_re: *mut WORD32 = 0 as *mut WORD32;
    let mut p_x_im: *mut WORD32 = 0 as *mut WORD32;
    let mut hyb_bands: WORD32 = hybrid_bands;
    ...
    rout_real_ptr = (*pstr_mps_state).mps_scratch_mem_v as *mut WORD32;
    rout_kernel_real_ptr = rout_real_ptr
        .offset(
            ((5112 as usize)
    ...
    rout_kernel_imag_ptr = rout_imag_ptr
        .offset(
            ((5112 as usize)
                .wrapping_mul(::core::mem::size_of::<WORD32>() as usize)
                .wrapping_add((8 as core::ffi::c_int - 1 as core::ffi::c_int) as usize)
    ...
    while row < v_channels {
        v_real = p_buffer_real;
        v_imag = p_buffer_imag;
        ts = 0 as core::ffi::c_int as WORD32;
        while ts < time_slots {
            memset(
                v_real as *mut core::ffi::c_void,
                0 as core::ffi::c_int,
                (hybrid_bands as size_t)
                    .wrapping_mul(::core::mem::size_of::<WORD32>() as size_t),
            );
            memset(
                v_imag as *mut core::ffi::c_void,
                0 as core::ffi::c_int,
                (hybrid_bands as size_t)
                    .wrapping_mul(::core::mem::size_of::<WORD32>() as size_t),
            );
            v_real = v_real.offset(MAX_HYBRID_BANDS as isize);
            v_imag = v_imag.offset(MAX_HYBRID_BANDS as isize);
            ts += 1;
        }
    ...
    if imag_present == 0 {
        ...
        while row < v_channels {
            ...
            while col < x_channels {
                if (*pstr_mps_state).m1_param_present[row as usize][col as usize] != 0 {
                    let mut idx: WORD32 = *index.offset(col as isize);
                    ixheaacd_dec_interp_umx(
                        ((*m1_param).m1_param_real[row as usize][col as usize])
                            .as_mut_ptr(),
                        rout_real_ptr,
                        m1_param_real_prev,
                        pstr_mps_state,
                    ...
                    while ts < time_slots {
                        ...
                        while qs < idx {
                            ...
                            *v_real = ixheaac_add32_sat(*v_real, temp_1);
                            v_real = v_real.offset(1);
                            temp_1 = ixheaacd_mps_mult32_shr_15(
                                *x_imag,
                                *rout_kernel_real_ptr,
                            );
                            rout_kernel_real_ptr = rout_kernel_real_ptr.offset(1);
                            *v_imag = ixheaac_add32_sat(*v_imag, temp_1);
                            v_imag = v_imag.offset(1);
                            x_real = x_real.offset(1);
                            x_imag = x_imag.offset(1);
                            qs += 1;
                        }
                        rout_kernel_real_ptr = rout_kernel_real_ptr
                            .offset((hyb_bands - idx) as isize);
                        p_v_real = p_v_real.offset(MAX_HYBRID_BANDS as isize);
                        p_v_imag = p_v_imag.offset(MAX_HYBRID_BANDS as isize);
                        p_x_re = p_x_re.offset(MAX_HYBRID_BANDS as isize);
                        p_x_im = p_x_im.offset(MAX_HYBRID_BANDS as isize);
                        ts += 1;
                    }
                    m1_param_real_prev = m1_param_real_prev
        ...
        row = 0 as core::ffi::c_int as WORD32;
        while row < v_channels {
            let mut p_x_real_0: *mut WORD32 = (*p_array_struct).x_real;
            let mut p_x_imag_0: *mut WORD32 = (*p_array_struct).x_imag;
            col = 0 as core::ffi::c_int as WORD32;
            while col < x_channels {
                if (*pstr_mps_state).m1_param_present[row as usize][col as usize] != 0 {
                    let mut idx_0: WORD32 = *index.offset(col as isize);
                    ixheaacd_dec_interp_umx(
                        ((*m1_param).m1_param_real[row as usize][col as usize])
                            .as_mut_ptr(),
                        rout_real_ptr,
                    ...
                    ixheaacd_dec_interp_umx(
                        ((*m1_param).m1_param_imag[row as usize][col as usize])
                            .as_mut_ptr(),
                        rout_imag_ptr,
                        m1_param_imag_prev,
                        pstr_mps_state,
                    ...
                    while ts < time_slots {
                        ...
                        while qs < 2 as core::ffi::c_int {
                            ...
                            *v_real = ixheaac_add32_sat(*v_real, temp_1);
                            ...
                            rout_kernel_imag_ptr = rout_kernel_imag_ptr.offset(1);
                        ...
                        rout_kernel_real_ptr = rout_kernel_real_ptr
                            .offset((hyb_bands - idx_0) as isize);
                        rout_kernel_imag_ptr = rout_kernel_imag_ptr
                            .offset((hyb_bands - idx_0) as isize);
                        p_v_real = p_v_real.offset(MAX_HYBRID_BANDS as isize);
                        p_v_imag = p_v_imag.offset(MAX_HYBRID_BANDS as isize);
                        p_x_re = p_x_re.offset(MAX_HYBRID_BANDS as isize);
                    ...
                    m1_param_real_prev = m1_param_real_prev
                        .offset(num_parameter_bands as isize);
                    m1_param_imag_prev = m1_param_imag_prev
                        .offset(num_parameter_bands as isize);
                    p_x_real_0 = p_x_real_0.offset(TSXHB as isize);
                    p_x_imag_0 = p_x_imag_0.offset(TSXHB as isize);
                } else {
                    m1_param_real_prev = m1_param_real_prev
                        .offset(num_parameter_bands as isize);
                    m1_param_imag_prev = m1_param_imag_prev
