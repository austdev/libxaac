...
pub const ONE_IN_Q28: core::ffi::c_int = 268435456 as core::ffi::c_int;
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_dec_interp_umx(
    mut m: *mut [WORD32; 28],
    mut ptr_r: *mut WORD32,
    mut ptr_m_prev: *mut WORD32,
    mut pstr_mps_state: *mut ia_heaac_mps_state_struct,
) -> VOID {
    let mut ts: WORD32 = 0;
    let mut ps: WORD32 = 0 as WORD32;
    let mut pb: WORD32 = 0;
    let mut r_out: *mut WORD32 = 0 as *mut WORD32;
    ...
    let mut prev_slot: WORD32 = -(1 as WORD32);
    let mut prm_slot: *mut WORD32 = ((*(*pstr_mps_state).aux_struct).param_slot)
        .as_mut_ptr();
    let mut curr_slot: WORD32 = *prm_slot;
    let mut temp: WORD32 = *reciprocal_tab.offset(curr_slot as isize);
    r_out = ptr_r;
    ts = (prev_slot as core::ffi::c_int + 1 as core::ffi::c_int) as WORD32;
    while ts <= curr_slot {
        pb = 0 as core::ffi::c_int as WORD32;
        while pb < num_parameter_bands {
            let mut alpha: WORD32 = temp * (ts + 1 as WORD32);
            let mut one_minus_alpha: WORD32 = 0;
            let mut result: WORD64 = 0;
            one_minus_alpha = ONE_IN_Q28 - alpha;
            result = *ptr_m_prev as WORD64 * one_minus_alpha as WORD64;
            result += (*m.offset(ps as isize))[pb as usize] as WORD64 * alpha as WORD64;
            let fresh0 = r_out;
            r_out = r_out.offset(1);
            *fresh0 = (result >> 28 as core::ffi::c_int) as WORD32;
            ptr_m_prev = ptr_m_prev.offset(1);
            pb += 1;
        }
        ptr_m_prev = ptr_m_prev.offset(-(num_parameter_bands as isize));
        ts += 1;
    }
    ps = 1 as core::ffi::c_int as WORD32;
    while ps < num_parameter_sets {
        let mut prev_slot_0: WORD32 = *prm_slot
            .offset((ps as core::ffi::c_int - 1 as core::ffi::c_int) as isize);
        let mut curr_slot_0: WORD32 = *prm_slot.offset(ps as isize);
        temp = *reciprocal_tab
            .offset(
                (curr_slot_0 as core::ffi::c_int - prev_slot_0 as core::ffi::c_int
                    - 1 as core::ffi::c_int) as isize,
            );
        ts = (prev_slot_0 as core::ffi::c_int + 1 as core::ffi::c_int) as WORD32;
        while ts <= curr_slot_0 {
            pb = 0 as core::ffi::c_int as WORD32;
            while pb < num_parameter_bands {
                let mut alpha_0: WORD32 = (ts - prev_slot_0) * temp;
                let mut result_0: WORD64 = 0;
                let mut one_minus_alpha_0: WORD32 = 0;
                one_minus_alpha_0 = ONE_IN_Q28 - alpha_0;
                result_0 = (*m
                    .offset(
                        (ps as core::ffi::c_int - 1 as core::ffi::c_int) as isize,
                    ))[pb as usize] as WORD64 * one_minus_alpha_0 as WORD64;
                result_0
                    += (*m.offset(ps as isize))[pb as usize] as WORD64
                        * alpha_0 as WORD64;
                let fresh1 = r_out;
                r_out = r_out.offset(1);
                *fresh1 = (result_0 >> 28 as core::ffi::c_int) as WORD32;
                pb += 1;
            }
            ts += 1;
        }
...
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_apply_abs_kernels(
    mut ptr_r_in: *mut WORD32,
    mut ptr_r_out: *mut WORD32,
    mut ptr_params: *mut size_t,
) -> VOID {
    let mut ts: WORD32 = 0;
    let mut qb: WORD32 = 0;
    let mut idx_ptr: *mut size_t = *ptr_params.offset(0 as core::ffi::c_int as isize)
        as *mut size_t;
    let mut time_slots: WORD32 = *ptr_params.offset(1 as core::ffi::c_int as isize)
        as WORD32;
    let mut num_parameter_bands: WORD32 = *ptr_params
        .offset(2 as core::ffi::c_int as isize) as WORD32;
    let mut hybrid_bands: WORD32 = *ptr_params.offset(3 as core::ffi::c_int as isize)
        as WORD32;
    ts = 0 as core::ffi::c_int as WORD32;
    while ts < time_slots {
        let mut idx: *mut size_t = idx_ptr;
        qb = 0 as core::ffi::c_int as WORD32;
        while qb < hybrid_bands {
            let mut idx_v: WORD32 = *idx as WORD32;
            let fresh2 = ptr_r_out;
            ptr_r_out = ptr_r_out.offset(1);
            *fresh2 = *ptr_r_in.offset(idx_v as isize);
            idx = idx.offset(1);
            qb += 1;
        }
        ptr_r_in = ptr_r_in.offset(num_parameter_bands as isize);
        ts += 1;
    }
}
