extern "C" {
    ...
    static ixheaacd_hybrid_band_71_to_processing_band_20_map: [WORD32; 71];
}
...
pub const ABS_THR: core::ffi::c_float = 1e-9f32
    ...
    * 32768 as core::ffi::c_int as core::ffi::c_float;
pub const DIR_DIFF_IN: core::ffi::c_int = 0 as core::ffi::c_int;
pub const DOWNMIX_IN: core::ffi::c_int = 1;
pub const GES_ALPHA: core::ffi::c_float = 0.99637864f32;
pub const GES_BETA: core::ffi::c_float = 0.9643691f32;
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_mps_env_init(
    mut self_0: *mut ia_mps_dec_state_struct,
) -> VOID {
    let mut i: WORD32 = 0;
    i = 0 as core::ffi::c_int as WORD32;
    while i < 3 as core::ffi::c_int {
        (*self_0).guided_env_shaping.avg_energy_prev[i as usize] = (32768.0f32
            * 32768.0f32) as FLOAT32;
        i += 1;
    }
}
unsafe extern "C" fn ixheaacd_mps_est_normalized_envelope(
    mut self_0: *mut ia_mps_dec_state_struct,
    mut inp: WORD32,
    mut ch: WORD32,
    mut env: *mut FLOAT32,
) -> VOID {
    let mut slot_energy: [[FLOAT32; 28]; 72] = [
        [
    ...
    let mut whitening_weight: [FLOAT32; 28] = [0.; 28];
    let mut ii: WORD32 = 0;
    let mut jj: WORD32 = 0;
    let mut param_band: WORD32 = 0;
    let mut k_start: WORD32 = 10 as WORD32;
    let mut k_stop: WORD32 = 18 as WORD32;
    let mut total_energy: FLOAT32 = 0 as core::ffi::c_int as FLOAT32;
    let mut avg_energy: FLOAT32 = 0 as core::ffi::c_int as FLOAT32;
    let mut ch_offset: WORD32 = 0;
    match inp {
        DIR_DIFF_IN => {
            ch_offset = 0 as core::ffi::c_int as WORD32;
            ii = 0 as core::ffi::c_int as WORD32;
            while ii < (*self_0).time_slots {
                jj = 0 as core::ffi::c_int as WORD32;
                while jj < (*self_0).hyb_band_count_max {
                    slot_energy[ii
                        as usize][ixheaacd_hybrid_band_71_to_processing_band_20_map[jj
                        as usize] as usize]
                        += ((*self_0)
                            .hyb_dir_out[ch as usize][ii as usize][jj as usize]
                            .re
                            + (*self_0)
                                .hyb_diff_out[ch as usize][ii as usize][jj as usize]
                                .re)
                            * ((*self_0)
                                .hyb_dir_out[ch as usize][ii as usize][jj as usize]
                                .re
                ...
                while ii < (*self_0).time_slots {
                    ...
                    while jj < (*self_0).hyb_band_count_max {
                        slot_energy[ii
                            as usize][ixheaacd_hybrid_band_71_to_processing_band_20_map[jj
                            as usize] as usize]
                            += (*self_0).hyb_in[ch as usize][jj as usize][ii as usize].re
                                * (*self_0).hyb_in[ch as usize][jj as usize][ii as usize].re
                                + (*self_0).hyb_in[ch as usize][jj as usize][ii as usize].im
                                    * (*self_0)
                                        .hyb_in[ch as usize][jj as usize][ii as usize]
                                        .im;
                        jj += 1;
                    }
                    ii += 1;
                ...
                while ii < (*self_0).time_slots {
                    ...
                    while jj < (*self_0).hyb_band_count_max {
                        slot_energy[ii
                            as usize][ixheaacd_hybrid_band_71_to_processing_band_20_map[jj
                            as usize] as usize]
                            += (*self_0).w_dir[ch as usize][ii as usize][jj as usize].re
                                * (*self_0).w_dir[ch as usize][ii as usize][jj as usize].re
                                + (*self_0).w_dir[ch as usize][ii as usize][jj as usize].im
                                    * (*self_0).w_dir[ch as usize][ii as usize][jj as usize].im;
                        jj += 1;
                    }
                    ii += 1;
    ...
    param_band = k_start;
    while param_band <= k_stop {
        pb_energy[param_band as usize] = (*self_0)
            .guided_env_shaping
            .pb_energy_prev[(ch + ch_offset) as usize][param_band as usize];
        param_band += 1;
    }
    avg_energy = (*self_0).guided_env_shaping.avg_energy_prev[(ch + ch_offset) as usize];
    ii = 0 as core::ffi::c_int as WORD32;
    while ii < (*self_0).time_slots {
        total_energy = 0 as core::ffi::c_int as FLOAT32;
        param_band = k_start;
        while param_band <= k_stop {
            pb_energy[param_band as usize] = (1 as core::ffi::c_int as FLOAT32
                - GES_ALPHA) * slot_energy[ii as usize][param_band as usize]
                + GES_ALPHA * pb_energy[param_band as usize];
            total_energy += slot_energy[ii as usize][param_band as usize];
            param_band += 1;
        }
        total_energy
            /= (k_stop as core::ffi::c_int - k_start as core::ffi::c_int
                + 1 as core::ffi::c_int) as FLOAT32;
        total_energy = (1 as core::ffi::c_int as FLOAT32 - GES_ALPHA) * total_energy
            + GES_ALPHA
                * (*self_0)
                    .guided_env_shaping
                    .frame_energy_prev[(ch + ch_offset) as usize];
        (*self_0).guided_env_shaping.frame_energy_prev[(ch + ch_offset) as usize] = total_energy;
        param_band = k_start;
        while param_band <= k_stop {
            whitening_weight[param_band as usize] = (total_energy as core::ffi::c_float
                / (pb_energy[param_band as usize] + ABS_THR)) as FLOAT32;
            param_band += 1;
        }
        *env.offset(ii as isize) = 0 as core::ffi::c_int as FLOAT32;
        param_band = k_start;
        while param_band <= k_stop {
            *env.offset(ii as isize)
                += slot_energy[ii as usize][param_band as usize]
                    * whitening_weight[param_band as usize];
            param_band += 1;
        }
        avg_energy = (1 as core::ffi::c_int as FLOAT32 - GES_BETA)
            * *env.offset(ii as isize) + GES_BETA * avg_energy;
        *env.offset(ii as isize) = sqrt(
            (*env.offset(ii as isize) / (avg_energy as core::ffi::c_float + ABS_THR))
    ...
    while param_band <= k_stop {
        (*self_0)
            .guided_env_shaping
            .pb_energy_prev[(ch + ch_offset) as usize][param_band as usize] = pb_energy[param_band
            as usize];
        param_band += 1;
    }
    (*self_0).guided_env_shaping.avg_energy_prev[(ch + ch_offset) as usize] = avg_energy;
}
#[no_mangle]
pub unsafe extern "C" fn ixheaacd_mps_time_env_shaping(
    mut self_0: *mut ia_mps_dec_state_struct,
) -> VOID {
    let mut dir_energy: [FLOAT32; 72] = [0.; 72];
    let mut dmx_energy: [FLOAT32; 72] = [0.; 72];
    let mut ch: WORD32 = 0;
    ...
    let mut amp_direct: FLOAT32 = 0 as core::ffi::c_int as FLOAT32;
    let mut amp_diff: FLOAT32 = 0 as core::ffi::c_int as FLOAT32;
    let mut amp_ratio: FLOAT32 = 0.;
    band_start = 6 as core::ffi::c_int as WORD32;
    ixheaacd_mps_est_normalized_envelope(
        self_0,
        DOWNMIX_IN,
        0 as WORD32,
        dmx_energy.as_mut_ptr(),
    );
    ...
    while ch < (*self_0).out_ch_count {
        ixheaacd_mps_est_normalized_envelope(
            ...
            DIR_DIFF_IN,
            ch,
            dir_energy.as_mut_ptr(),
        );
        if (*self_0).temp_shape_enable_ch_ges[ch as usize] != 0 {
            time_slot = 0 as core::ffi::c_int as WORD32;
            while time_slot < (*self_0).time_slots {
                gain = ((*self_0).env_shape_data[ch as usize][time_slot as usize]
                    * dmx_energy[time_slot as usize]
                    / (dir_energy[time_slot as usize] + 1e-9f32)) as FLOAT32;
                amp_direct = 0 as core::ffi::c_int as FLOAT32;
                amp_diff = 0 as core::ffi::c_int as FLOAT32;
                jj = band_start;
                while jj < (*self_0).hyb_band_count_max {
                    amp_direct
                        += (*self_0)
                            .hyb_dir_out[ch as usize][time_slot as usize][jj as usize]
                            .re
                            * (*self_0)
                                .hyb_dir_out[ch as usize][time_slot as usize][jj as usize]
                                .re
                    ...
                    amp_diff
                        += (*self_0)
                            .hyb_diff_out[ch as usize][time_slot as usize][jj as usize]
                            .re
                            * (*self_0)
                                .hyb_diff_out[ch as usize][time_slot as usize][jj as usize]
                                .re
                ...
                amp_ratio = sqrt(
                    (amp_diff as core::ffi::c_float
                        / (amp_direct as core::ffi::c_float + ABS_THR))
                        as core::ffi::c_double,
                ) as FLOAT32;
                ratio = (if (if gain
                    + amp_ratio * (gain - 1 as core::ffi::c_int as FLOAT32)
                    > 1 as core::ffi::c_int as core::ffi::c_float / 4.0f32
                {
                    gain as core::ffi::c_float
                        + amp_ratio as core::ffi::c_float
                            * (gain as core::ffi::c_float
                ...
                {
                    if gain + amp_ratio * (gain - 1 as core::ffi::c_int as FLOAT32)
                        > 1 as core::ffi::c_int as core::ffi::c_float / 4.0f32
                    {
                        gain as core::ffi::c_float
                            + amp_ratio as core::ffi::c_float
